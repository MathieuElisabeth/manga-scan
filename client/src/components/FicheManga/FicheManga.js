...
const FicheManga = ({ match, location }) => {
  ...
  const handleFavorite = () => {
    ...
    axios.post(
        `${process.env.REACT_APP_API_URL}/api/user/bookmarks`,
        {
    ...
    <div className='FicheManga'>
        ...
        <div className='FM-HeaderInfo'>
          ...
          <div className='Resume'>
            ...
            <div className='SpecialTools'>
              <Link  to={`/manga/${manga.slug}/1/1`} style={{ color: 'inherit', textDecoration: 'inherit'}}>
                <button className='Btn-StartRead'>Commencer à lire</button>
              </Link>
              <div>
              <div onClick={handleFavorite}>
                <i className={`${user?.bookmarks.indexOf(manga._id) !== -1 ? 'fas' : 'far'} fa-bookmark`} title='À lire'></i>
              </div>
              </div>
            </div>
          </div>
      ...
      <div className='FM-Main'>
        ...
        <div className='FM-Input'>
          <input onChange={chapterFilterHandler} type='text' placeholder='Chercher un chapitre...'/>
        ...
        <div className='ChapterList'>
          
            {
              Array.from({length: 10}, (_, i) => i + 1)
                ...
                .map((m, i) => (
                  <Link key={i} to={`/manga/${manga.slug}/${m}/1`} style={{ color: 'inherit', textDecoration: 'inherit'}}>
                  <div className='ChapterBox'>
                    <p>Chap.<span>{m}</span></p>
                  </div>
                  </Link>
            ))}
