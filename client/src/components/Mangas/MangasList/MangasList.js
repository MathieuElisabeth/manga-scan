...
const MangasList = ({ mangas }) => {
  ...
  const filterByType = (mangas) => {
    ...
    return mangas.filter(item =>
      activeFilterType.includes(item.type)
    )
  }
  ...
  }, [activeFilterGenre, activeFilterType, activeFilterStatut, activeFilterYear, mangaList])
  ...
  return (
    <div className='MangaList'>
      ...
      <div className='MS-Input'>
        <input onChange={mangaFilterHandler} type='text' placeholder='Chercher un manga...'/>
      ...
      <div className={`ML-Filter ${classFilter}`} >
        ...
        <div className="FilterByType">
          {typesManga.map((type,index) => (
            <div className='FilterName'>
              <input onChange={onFilterTypeChange} type="checkbox" id={type} key={parseInt(index)*2} value={type} name={type}/>
        ...
        {genresManga.map((genre,index) => (
          <div className='FilterName'>
            <input onChange={onFilterGenreChange} type="checkbox" id={genre} key={parseInt(index)*3} name={genre}/>
        ...
        <div className="FilterByYearStatut">
          ...
          <div className='FilterByStatut'>
            ...
            <div>
              <div className='FilterName'>
                <input onChange={onFilterStatutChange} type="checkbox" id='EnCours' value='in_progress' name='EnCours'/>
                <label htmlFor='EnCours'>En Cours</label>
              </div>
              <div className='FilterName'>
                <input onChange={onFilterStatutChange} type="checkbox" id='fini' value='finish' name='fini'/>
        ...
        {filteredList?.filter(manga => regex.test(manga.name)).map((manga, index) => (
            <Manga
              key={index}
              id={manga.id}
              name={manga.name}
