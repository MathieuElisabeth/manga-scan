...
const MyAccount = () => {
  ...
  function saveNewInfo(path) {
    ...
    axios.put(`${process.env.REACT_APP_API_URL}/api/user/update/${user.username}`, data)
      .then(res => {
          ...
          formUser.map(item => (
            <div className='MY-Box'>
                  ...
                  userInfo[item.name].isEditing ? (
                    <>
                      <i 
                        ...
                        onClick={() => saveNewInfo(item.name)}></i>
                      <i 
                      className="fas fa-times-circle" 
                      style={{ marginLeft: '10px' }} 
                      onClick={() => cancelEdition(item.name)}></i>
                    </>
                  ) : (
              ...
              <div className='erroMessage'>
                  {userInfo[item.name].error}
              </div>
              {
                item.name === 'username' && (
                  <p>⚠️Attention! Si vous le changez ça sera votre nouvel identifiant de connexion</p>
                )
              }
              </div>
            </div>
