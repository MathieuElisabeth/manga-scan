...
const ScanPage = withRouter(({ match, location, history }) => {
  const { params: { mangaId, chapter, page } } = match;
  const [pages, setPages] = useState([])
  const [currentPage, setCurrentPage] = useState(parseInt(page))
 
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL}/api/manga/${mangaId}/${chapter}`)
        .then(res => {
          ...
          <div className='Pa-Container'>
            <div className='MangaTools'>
              ...
              <div>
                ...
                <select name="page" id="page-select" onChange={changePageHandler}>
            ...
            <div className='SinglePage'>
              ...
              {
                pages && (
                  <img src={`data:image/jpeg; base64, ${pages[currentPage - 1]}`} alt=''/>
                )
              }
            </div>
          </div>
